...
class KnowledgeBaseService:
    ...
    def answer(
        ...
        if question_type == "should_refuse":
            reasoning.append("guard=policy_refusal")
            return AnswerResult(
                answer=self._attach_citations(self.question_policy.build_refusal_answer(), citations),
        ...
        if not grounded or self.question_policy.is_low_confidence(question, hits, question_type):
            ...
            return AnswerResult(
                answer=self._attach_citations(self.question_policy.build_low_confidence_answer(), citations),
                grounded=False,
                citations=citations,
                reasoning=reasoning,
                question_type=question_type,
        ...
        return AnswerResult(
            ...
            reasoning=reasoning,
            question_type=question_type,
            confidence="medium" if question_type != "complex_reasoning" else "low",
